<template>
  <b-container class="mt-3">
    ...
    <b-row v-if="!!allQueryData">
      <b-col v-for="(item,i) in allQueryData" :key="i">
...
export default {
  ...
  async beforeMount() {
    ...
    if (this.$localStore.get('searchQuery') === this.id) {
    ...
    try {
      await this.$store.dispatch('act_getAllDataByQuery', this.id.toString() || this.$localStore.get('rQuetionId'));
    } catch (error) {
